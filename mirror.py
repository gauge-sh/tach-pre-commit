...
def get_all_versions() -> list[Version]:
    response = urllib3.request("GET", "https://pypi.org/pypi/tach/json")
...
def get_current_version(pyproject: dict) -> Version:
    ...
    requirement = next((r for r in requirements if r.name == "tach"), None)
    assert requirement is not None, "pyproject.toml does not have tach requirement"

    specifiers = list(requirement.specifier)
    assert (
        len(specifiers) == 1 and specifiers[0].operator == "=="
    ), f"tach's specifier should be exact matching, but `{requirement}`"
